...
```
Then I opened the GUI of VICTIM, launched the File Browser and just input `\\casualname\` without any other key, that I was greeted with those messages on the ntlmrelayx console: 
...
This way the tool understands that you want to use the open connection that was already intercepted before, hence it uses the NTLM authentication relayed in the previous step, bypassing the password requirement.

![proxychain](images/proxychain.png)

Please note that even if the attacked users are not admin of the system, there could still be some overprivilege problems that an attacker could investigate and leverage, like for example a user could access some network share with confidential data. Lets keep this in mind for later.
...
This means that systems periodically poll for an IPv6 lease and, if found, it will immediately start to use it since IPv6 has precedence over IPv4.

Let's start mitm6 on our attacker computer: 
```
mitm6 -d ntlmlab.local --ignore-nofqdn
```
We are now assigning IPv6 addresses to computers requesting DHCPv6 leases over the wire. Once poison is complete, when a victim user attempts to browse an internal resource, which is to say something that requires internal DNS resolution, it will be resolved by our tool with the address of attacker, and on the victim machine will appear a prompt asking for credentials which totally seems legit; this works on internet explorer and other browsers that do transparent proxying.

This is what happens on the victim machine:
- on the left the result of a ipconfig /all to show that there is a new DNS server with IPv6 address, and the spoofed response for pinging zabbix.ntlmlab.local to the attacker’s IP address 10.0.0.7;
...
This is what happens on the attacker machine, on the left the mitm6 tool spoofing addresses, on the right ntlmrelayx:

![ntlmrelaycrossprot](images/ntlmrelaycrossprot.png)

When the victim enters his credentials, the attacker will get a successfull cross-protocol relay, from an unauthenticated standpoint.
